...
def optimize_delta(x, actor, dynamic_model, state_dim, device, args, steps=1, lr=0.05, iters=10): 
...
def execute_adversarial_safety_attack(actor, surrogate_actor, dynamic_model, attack_time, envs, device, args):
    ...
    for t in range(args.max_episode_length):
        if t > attack_time:
            if not recorded:
                ...
                recorded = True
            obs_delta = optimize_delta(obs, surrogate_actor, dynamic_model, state_dim, device, args, 
...
def build_state_value_dataset(envs, actor, surrogate_actor, dynamic_model, device, args, attack_time_list):
    all_obs, all_acts, all_next_obs, all_rews = [], [], [], []
    data = []
    progress_bar = tqdm(attack_time_list, desc="Processing")
    for attack_time in progress_bar:
        episode_data, obs, P_succ, E_step = execute_adversarial_safety_attack(actor, surrogate_actor, dynamic_model, attack_time, envs, device, args)
        if obs is not None:
...
def evaluate_MPA(actor, surrogate_actor, dynamic_model, value_model, threshold, envs, device, args):
    H = args.max_pert_length  
    ...
    for iteration in range(args.num_iterations):
        ...
        for t in range(args.max_episode_length):
            ...
            if whe_attack_finished == 0:
                attack_not_success += 1
                break
    # log_results_black_box
    log_filename = f"./log_results_black_box/log_{args.env_id}_{args.attack_flag}_{args.Horizon}.log"
...
def main(args):
    ...
    surrogate_actor = CloneActor(envs).to(device)
    surrogate_actor.load_model(f"./surrogate_models/SM_{args.env_id}_{args.attack_flag}.pth")
    ...
    # load dynamic model
    save_path = f'/home/mzm/RL_codes/BreSTL_clean/normalization_stats/normalization_stats_{args.env_id}.json'
    ...
    if args.train_or_test == "train-value":
        ...
        _, dataset = build_state_value_dataset(envs, actor, surrogate_actor, dynamic_model, device, args, attack_time_list)
        ...
        value_model.save_model(f"./value_models/NEW_B_VF_{args.env_id}_{args.attack_flag}.pth")
        
    elif args.train_or_test == "test":
        value_model = ValueNet(state_dim).to(device)
        value_model.set_normalizer(state_mean, state_std)
        value_model = value_model.load_model(f"./value_models/B_VF_{args.env_id}_{args.attack_flag}.pth", device='cuda')
        dynamic_model.load_model(f"./dynamics_models/DM_{args.env_id}_{args.attack_flag}.pth")
        ...
        # print("the thredshold is ", threshold)
        args.threshold = threshold
        evaluate_MPA(actor, surrogate_actor, dynamic_model, value_model, threshold, envs, device, args)
