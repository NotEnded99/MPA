...
from utils import set_seed, load_normalization_stats, make_env, save_normalization_stats
from agent.sac_agent import Actor, load_actor

parser = argparse.ArgumentParser()
parser.add_argument("--env_id", type=str, default="go2")
...
parser.add_argument("--dataset_path", type=str, default="/data/mzm/real_robot/transitions_dataset_go2_8000.pt")
...
def train_model(args):
    ...
    # states = transitions['states'][:, 0:12,15:26]
    states = torch.cat([transitions['states'][:, 0:12], transitions['states'][:, 15:39]], dim=1)
    print(states.shape)
    # a
    actions = transitions['actions']
    rewards = transitions['rewards']
    #  = transitions['next_states']
    next_states = torch.cat([transitions['next_states'][:, 0:12], transitions['next_states'][:, 15:39]], dim=1)
    deltas = next_states - states

    model = StableDynamicsModel(state_dim, action_dim, args.hidden_size, device=device).to(device)
    ...
    save_normalization_stats(args.env_id, state_mean, state_std, action_mean, 
                             action_std, delta_mean, delta_std, reward_mean, reward_std)
